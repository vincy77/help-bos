...
export class ShowcaseService {
  ...
  private categoryDataUrl = 'api/categoryData';
  ...
  getCategoryNav4(id: string): Promise<any> {
    const url = `${this.categoryDataUrl}/${id}`;
  ...
  // 返回对应id发对象
  getCategoryData(id: string): Promise<any> {
    const url = `${this.categoryDataUrl}/${id}`;
    console.log(url);
  ...
  getCategoryNav2(id: string): Promise<any> {
    //const url = `${this.categoryDataUrl}/${id}`;
    return this.http.get(this.categoryDataUrl + '?id=' + id)
      .toPromise()
      .then(response => {
        console.log(response.json());
        return response.json();
  ...
  getCategoryNav(data: any): Promise<any> {
    console.log(data);
    return this.http.get(this.categoryDataUrl)
